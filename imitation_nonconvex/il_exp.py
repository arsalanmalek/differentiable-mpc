...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="nn", choices=["nn", "empc", "sysid"]
    )
...
class IL_Exp:
        ...
        #     self.dx = self.dx.to(self.device)
            ...
            if self.learn_dx:
                if self.env_name == "pendulum":
                    self.env_params = torch.tensor((15.0, 3.0, 0.5), requires_grad=True)
                    ...
                    torch.manual_seed(self.seed)
                    self.env_params = torch.tensor((5.0, 1.0, 1.0)) + torch.tensor(
                        (3.0, 1.0, 1.0)
                    ) * (torch.rand(3) - 0.5)
    ...
    def run(self):
        ...
        if self.learn_dx:
            ...
            dx_f.write(
                ",".join(
                    map(str, self.env.true_dx.params.cpu().detach().numpy().tolist())
                )
            )
            dx_f.write("\n")
            dx_f.flush()
        ...
        if self.mode in ["empc", "sysid"]:
            ...
            val_warmstart = torch.zeros(self.env.val_data.shape[0], T, self.n_ctrl).to(
                self.device
            )
            test_warmstart = torch.zeros(
                self.env.test_data.shape[0], T, self.n_ctrl
            ).to(self.device)
        else:
        ...
        for i in range(self.n_epoch):
            ...
            if self.mode in ["empc", "sysid"] and i % self.restart_warmstart_every == 0:
            ...
            for j, (xinits, xs, us, idxs) in enumerate(train):
                ...
                if self.learn_dx:
                    ...
                    dx_f.write(
                        ",".join(
                            map(str, self.env_params.cpu().detach().numpy().tolist())
                        )
                    )
                    dx_f.write("\n")
                    dx_f.flush()
                if self.learn_cost:
                    ...
                    cost_f.write(
                        ",".join(
                            map(str, torch.cat((q, p)).cpu().detach().numpy().tolist())
                        )
                    )
                    cost_f.write("\n")
                    cost_f.flush()
