...
class IL_Env:
    def __init__(self, env, lqr_iter=500, mpc_T=20, slew_rate_penalty=None):
        ...
        if self.env == "pendulum":
            ...
            params = torch.tensor((10.0, 1.0, 1.0, 1.0, 0.1))
    ...
    def mpc(
        self, dx, xinit, q, p, u_init=None, eps_override=None, lqr_iter_override=None
    ):
        n_batch = xinit.shape[0]

        n_sc = self.true_dx.n_state + self.true_dx.n_ctrl

        Q = torch.diag(q).unsqueeze(0).unsqueeze(0).repeat(self.mpc_T, n_batch, 1, 1)
